...
def setup_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS negociacoes (
            id INTEGER PRIMARY KEY AUTOINCREMENT, ticker TEXT NOT NULL, trade_date TEXT,
            open_price REAL, min_price REAL, max_price REAL, close_price REAL
        )
    ''')
    conn.commit()
    conn.close()
    logging.info(f"Banco de dados '{DB_FILE}' e tabela 'negociacoes' prontos.")
...
def insert_stocks_data(stocks_data):
    ...
    conn.commit()
    cursor.executemany('INSERT INTO negociacoes (ticker, trade_date, open_price, min_price, max_price, close_price) VALUES (?, ?, ?, ?, ?, ?)', stocks_data)
    conn.commit()
    conn.close()
...
def transform_and_load():
    ...
    for _, elem in context:
        ...
        #aqui ele pega só mercado aberto pelo ticketer
