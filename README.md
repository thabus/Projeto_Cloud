...
* 📌 **1. Introdução**
    * 1.1. Descrição Geral do Sistema (Cloud e Propósito)
    * 1.2. Objetivos do Projeto
* 📝 **2. Requisitos e Restrições Arquiteturais**
    * 2.1. Requisitos Funcionais
    * 2.2. Requisitos Não Funcionais
    * 2.3. Restrições (Plataforma Azure Obrigatória)
* 📂 **3. Casos de Uso**
    * 3.1. Casos de Uso do Sistema
    * 3.2. Diagrama de Classes
    * 3.3. Detalhamento dos Componentes e Relacionamentos
...
* 🖼️ **6. Frontend Web para Visualização** 
...
Este é um sistema **integralmente desenvolvido e hospedado na plataforma Microsoft Azure**, projetado para a análise de cotações da Bolsa de Valores do Brasil (**B3**). O projeto aborda o desafio de processar os arquivos diários de cotações em **larga escala**. A solução propõe um **Pipeline de Dados Cloud completo (ETL)**, que automatiza a ingestão, transformação e carga dos dados, culminando na disponibilização das informações para consumo em **dashboards analíticos e um Frontend Web dedicado**, também hospedado na nuvem.
...
* **RF08: Disponibilização para Análise:** Os dados finais armazenados no banco de dados devem estar disponíveis para serem consumidos por ferramentas de visualização (como Power BI) **e pelo Frontend Web**.
...
* **Pré-condições:** * A infraestrutura na Azure (Storage, Data Factory, Function, SQL DB, Logic Apps) está devidamente configurada.
    * Um novo arquivo de cotações foi depositado na área de dados brutos do Azure Storage Account.
* **Fluxo Principal (Caminho Feliz):** 1. A chegada de um novo arquivo no Azure Storage aciona o pipeline no Azure Data Factory para iniciar o processamento.
...
* **Fluxo de Exceção:** 1. Se ocorrer um erro em qualquer etapa do pipeline do Data Factory (transformação ou carga), a execução é interrompida.
...
* **Pós-condições:** * Em caso de sucesso, os novos dados de cotações estão disponíveis para consulta no Azure SQL Database.
...
* **Pré-condições:** * O container Docker está devidamente configurado e pronto para ser executado.
    * O Azure Storage Account está criado e acessível.
* **Fluxo Principal:** 1. O Desenvolvedor/Operador executa o container Docker.
...
* **Fluxo de Exceção:** 1. Se o container Docker não conseguir ser executado (ex: erro de configuração do Docker na máquina local), o processo falha e uma mensagem de erro é exibida no terminal do Desenvolvedor.
...
* **Pós-condições:** * Sucesso: O arquivo de cotações está armazenado na área de dados brutos da nuvem, pronto para acionar o pipeline de processamento.
...
* **Ator Principal:** Analista de Dados / Usuário Web
* **Descrição:** Mostra como um usuário final consome os dados já processados pelo pipeline para criar visualizações e gerar insights, seja por meio de ferramentas analíticas ou pelo Frontend.
* **Pré-condições:** * O "Caso de Uso 1" foi executado com sucesso pelo menos uma vez.
    * Os dados de cotações estão armazenados no Azure SQL Database.
    * O Analista de Dados possui uma ferramenta de visualização (como Power BI) ou acesso ao Frontend Web.
* **Fluxo Principal:** 1. O Analista de Dados/Usuário acessa o Frontend Web ou utiliza uma ferramenta de visualização, como Power BI ou Synapse Analytics.
    2. É estabelecida uma conexão (direta ou indireta via API) com o Azure SQL Database que contém as cotações processadas.
    3. O usuário cria relatórios, dashboards ou visualiza os dados apresentados no Frontend.
* **Fluxo de Exceção:** 1. Se o acesso ao Azure SQL Database falhar, uma mensagem de erro de conexão é exibida para o usuário.
    2. Se uma consulta executada no banco de dados for inválida, o banco retornará um erro que será exibido na interface de consumo.
* **Pós-condições:** * Sucesso: As informações das cotações são apresentadas de forma visual, permitindo a análise.
    * Falha: O Analista de Dados não consegue acessar os dados e recebe uma mensagem de erro, precisando corrigir o problema para continuar.

### 3.2. Diagrama de Classes

O diagrama de classes a seguir modela os principais componentes do sistema e suas interações, com foco na classe central de dados, `Cotacao`.
...
A arquitetura do sistema é desenhada como um **pipeline de dados moderno e automatizado**, implementado inteiramente na **nuvem Microsoft Azure**. O projeto segue o padrão de **ETL** para processar arquivos de cotações da B3, desde sua ingestão até o armazenamento em um banco de dados relacional para análise e visualização. O fluxo arquitetural pode ser resumido em três fases principais:
...
Adicionalmente, o **Logic Apps** é utilizado para automatizar notificações e alertas. Os dados consolidados no SQL Database são consumidos pelo **Frontend Web** (Seção 6) e por ferramentas como o Power BI.
...
Este diagrama visualiza a arquitetura do pipeline de dados, detalhando o fluxo de informações entre os principais serviços da **Microsoft Azure** e os componentes do projeto. Ele representa a jornada dos dados, desde a ingestão simulada até a carga final para análise e consumo pelo Frontend.
...
1.  **Ingestão (Container Docker):** A simulação da extração dos arquivos de cotações da B3 é realizada por um **Container Docker**. Este contêiner envia os arquivos brutos para um contêiner específico no **Azure Blob Storage**, conhecido como *raw zone*.
2.  **Orquestração e Transformação (Azure Data Factory):** A chegada de um novo arquivo na *raw zone* atua como um gatilho para o **Azure Data Factory**. Ele orquestra o pipeline de ETL, lendo o arquivo bruto e aplicando as transformações necessárias para limpar e estruturar os dados. O resultado desse processamento é salvo em um segundo contêiner no mesmo Storage Account, a *processed zone*.
3.  **Carga Incremental (Azure Function):** Um segundo gatilho, desta vez na *processed zone*, aciona uma **Azure Function**. Esta função, desenvolvida em Python, é responsável por ler o arquivo processado e realizar a carga incremental dos dados na tabela **Cotacoes** do **Azure SQL Database**.
4.  **Monitoramento e Notificações (Logic Apps):** Paralelamente ao pipeline principal, o **Logic Apps** monitora o status da execução do Azure Data Factory. Em caso de sucesso ou falha, ele envia notificações automáticas por e-mail, garantindo a visibilidade e o controle sobre o processo.
5.  **Análise e Visualização (Azure SQL Database & Frontend):** Os dados, agora limpos e organizados no **Azure SQL Database**, estão prontos para serem consumidos por ferramentas de visualização, como o Power BI, e pelo **Frontend Web**, permitindo a criação de dashboards e relatórios analíticos.

---

## 🖼️ 6. Frontend Web para Visualização

O Frontend Web é o componente de consumo final do projeto, garantindo a visualização dos dados processados, conforme o Requisito Funcional RF08.
...
* **Deploy Cloud:** O Frontend é um aplicativo web **hospedado na nuvem Azure** (serviço recomendado: **Azure Static Web Apps** ou **Azure App Service**), garantindo que todo o sistema, da ingestão à apresentação, opere na mesma plataforma.
